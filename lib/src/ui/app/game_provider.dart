...
import 'package:geniozinho/src/data/repository/calculator_repository.dart';
import 'package:geniozinho/src/data/repository/complex_calcualtion_repository.dart';
import 'package:geniozinho/src/data/repository/correct_answer_repository.dart';
import 'package:geniozinho/src/data/repository/cube_root_repository.dart';
import 'package:geniozinho/src/data/repository/dual_repository.dart';
import 'package:geniozinho/src/data/repository/find_missing_repository.dart';
import 'package:geniozinho/src/data/repository/magic_triangle_repository.dart';
import 'package:geniozinho/src/data/repository/math_grid_repository.dart';
import 'package:geniozinho/src/data/repository/math_pairs_repository.dart';
import 'package:geniozinho/src/data/repository/mental_arithmetic_repository.dart';
import 'package:geniozinho/src/data/repository/number_pyramid_repository.dart';
import 'package:geniozinho/src/data/repository/picture_puzzle_repository.dart';
import 'package:geniozinho/src/data/repository/quick_calculation_repository.dart';
import 'package:geniozinho/src/data/repository/sign_repository.dart';
import 'package:geniozinho/src/data/repository/square_root_repository.dart';
import 'package:geniozinho/src/data/repository/true_false_repository.dart';
import 'package:geniozinho/src/ui/app/time_provider.dart';
...
class GameProvider<T> extends TimeProvider with WidgetsBindingObserver {
  ...
  void startGame({int? level, bool? isTimer}) async {
   ...
   debugPrint("list--${list.length}====");
    ...
    if (_homeViewModel.isFirstTime(gameCategoryType)) {
     ...
     debugPrint("isTimerStart==$isTimer");
      if (isTimer) {
        restartTimer();
        notifyListeners();
      }
   ...
   debugPrint("list12===${list.length}");
    ...
    if (index < 19) {
     ...
     debugPrint("index===$index");
      ...
      //
      //
      //  debugPrint("currentScore===$currentScore==${KeyUtil.getScoreUtil(
      //       gameCategoryType)}");
      // }
      currentState = list[index];
    } else {
  ...
  void rightAnswer() {
   debugPrint("currentScoreRight===$currentScore");
    oldScore = currentScore;
    currentScore = currentScore + KeyUtil.getScoreUtil(gameCategoryType);

    addCoin();

   debugPrint("currentScore===12 $currentScore");
    notifyListeners();
  }
  ...
  void pauseResumeGame() {
   ...
   debugPrint("dialogType====${dialogType}");
  }
  ...
  void showExitDialog() {
   debugPrint("dialog---true2");
    pauseTimer();
   debugPrint("dialog---true3");
    dialogType = DialogType.exit;
   debugPrint("dialog---true1");
    notifyListeners();
  }
